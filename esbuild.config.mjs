...
const OBSIDIAN_PLUGIN_PATH = OBSIDIAN_VAULT && existsSync(OBSIDIAN_VAULT)
  ? path.join(OBSIDIAN_VAULT, '.obsidian', 'plugins', 'claudian-multisel')
  : null;
...
const copyToObsidian = {
  ...
  setup(build) {
    build.onEnd((result) => {
      ...
      // Clean up the node_modules tree the old classic-level approach copied
      // here — group names are now read with a pure-JS LevelDB reader bundled
      // into main.js, so no runtime-resolved native deps are needed.
      const staleNativeRoot = path.join(OBSIDIAN_PLUGIN_PATH, 'node_modules');
      rmSync(staleNativeRoot, { recursive: true, force: true });
    });
  }
};
